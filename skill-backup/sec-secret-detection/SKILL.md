...
description: Detect, remediate, and prevent hardcoded secrets, credentials, API keys, and sensitive configuration in codebases. Use when reviewing code for hardcoded secrets, auditing .env files, reviewing git history, setting up secret scanning, or designing secure configuration management. Trigger for any mention of API keys, hardcoded credentials, .env, secret leakage, git history secrets, or pre-commit hooks for secrets.
---
