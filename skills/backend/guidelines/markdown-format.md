...
Compile all backend design thinking into a single, well-structured markdown file.
...
- [ ] **api-contract-design** output → fills: API Design
- [ ] **data-schema-design** output → fills: Data & Storage
- [ ] **auth-authz-design** output → fills: Security
- [ ] **async-queue-plan** output → fills: Async / Queue Design (if applicable)
- [ ] **query-optimization** output → fills: Performance Considerations
- [ ] **idempotency-edge-case-plan** output → fills: Edge Cases, Reliability
- [ ] **observability-plan** output → fills: Observability
- [ ] **dependency-risk-analysis** output → fills: Tradeoffs / Decisions
- [ ] **pattern-read** output → fills: Pattern alignment notes
