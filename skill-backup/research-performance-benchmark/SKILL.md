---
name: research-performance-benchmark
description: >
  Use this skill to research, find, and interpret performance benchmarks for libraries,
  frameworks, databases, or architectural approaches. Trigger when the user asks "how fast
  is X?", "can X handle our load?", "what's the throughput of X vs Y?", "will X meet our
  latency SLA?", "how does X perform at scale?", "what are the memory/CPU characteristics
  of X?", or any question where performance is a primary decision factor. Also trigger when
  the user gives a specific target (e.g., "we need < 50ms p99") and wants to know if a
  technology can meet it.
---
