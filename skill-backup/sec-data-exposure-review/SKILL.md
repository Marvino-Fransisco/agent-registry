---
name: sec-data-exposure-review
description: Review code and systems for sensitive data exposure risks including PII leakage, verbose error messages, insecure logging, over-fetching in APIs, and data exposure in frontend bundles. Use when auditing API responses, log configurations, error handling, frontend bundles, or database queries that may expose more data than needed. Trigger for mentions of PII, data leakage, sensitive data exposure, logging PII, over-fetching, or GDPR data handling concerns.
---
