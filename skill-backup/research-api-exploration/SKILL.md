---
name: research-api-exploration
description: >
  Use this skill to explore, document, and evaluate third-party APIs before integrating them.
  Trigger when the user asks "how does X API work?", "what are the limits of X API?",
  "can X API support our use case?", "what does the X API response look like?",
  "how do we authenticate with X?", "what's the rate limit on X?", "compare the APIs
  of X vs Y for our use case", or any request to understand a third-party service's
  API capabilities, constraints, and integration complexity before committing to it.
---
