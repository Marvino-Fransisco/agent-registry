...
description: >
  Use this skill to research known security vulnerabilities, advisories, and best practices
  for a technology, library, or system design. Trigger when the user asks "is X safe to use?",
  "are there known vulnerabilities in X?", "what are the security implications of X?",
  "how do companies secure this pattern?", "what's the OWASP guidance on X?", or any
  question involving authentication, authorization, encryption, input validation, secrets
  management, or dependency security. Also trigger when reviewing a technology choice and
  security is a key decision factor.
---
